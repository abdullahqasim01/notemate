...
import { FFmpegKit, ReturnCode } from '@palashakhenia/ffmpeg-kit-react-native-sf';
...
export function useFileUpload() {
  ...
  const convertVideoToAudio = async (videoUri: string): Promise<string | null> => {
    try {
      setConverting(true);
      ...
      if (!ReturnCode.isSuccess(returnCode)) {
        ...
        throw new Error(`FFmpeg failed: ${output?.substring(0, 200) || 'Unknown error'}`);
      }
  ...
  const uploadVideoForChat = async (chatId: string, videoUri: string) => {
    try {
      setError(null);

      // Step 1: Convert video to audio
      console.log('Converting video to audio...', videoUri);
      const audioUri = await convertVideoToAudio(videoUri);
